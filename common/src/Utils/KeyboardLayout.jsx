import JsxUtilsModule from "../../../utils/JsxUtilsModule";

export default class extends JsxUtilsModule {
	import() {
		global.KeyboardLayout = class KeyboardLayout {
			// noinspection NonAsciiCharacters
			static Layouts = [{
				'~':'Ё', '`':'ё', '@':'"', '#':'№', 'q':'й', 'w':'ц', 'e':'у', 'r':'к',
				't':'е', 'y':'н', 'u':'г', 'i':'ш', 'o':'щ', 'p':'з', 'a':'ф', 's':'ы',
				'd':'в', 'f':'а', 'g':'п', 'h':'р', 'j':'о', 'k':'л', 'l':'д', 'z':'я',
				'x':'ч', 'c':'с', 'v':'м', 'b':'и', 'n':'т', 'm':'ь', 'Q':'Й', 'W':'Ц',
				'E':'У', 'R':'К', 'T':'Е', 'Y':'Н', 'U':'Г', 'I':'Ш', 'O':'Щ', 'P':'З',
				'A':'Ф', 'S':'Ы', 'D':'В', 'F':'А', 'G':'П', 'H':'Р', 'J':'О', 'K':'Л',
				'L':'Д', 'Z':'Я', 'X':'Ч', 'C':'С', 'V':'М', 'B':'И', 'N':'Т', 'M':'Ь',
				'[':'х', '{':'Х', ']':'ъ', '}':'Ъ', ';':'ж', ':':'Ж', '\'':'э', '"':'Э',
				',':'б', '<':'Б', '.':'ю', '>':'Ю', '?':',', '&':'?',
				'/':'.'
			}, {
				'Ё':'~', 'ё':'`', '"':'@', '№':'#', 'й':'q', 'ц':'w', 'у':'e', 'к':'r',
				'е':'t', 'н':'y', 'г':'u', 'ш':'i', 'щ':'o', 'з':'p', 'ф':'a', 'ы':'s',
				'в':'d', 'а':'f', 'п':'g', 'р':'h', 'о':'j', 'л':'k', 'д':'l', 'я':'z',
				'ч':'x', 'с':'c', 'м':'v', 'и':'b', 'т':'n', 'ь':'m', 'Й':'Q', 'Ц':'W',
				'У':'E', 'К':'R', 'Е':'T', 'Н':'Y', 'Г':'U', 'Ш':'I', 'Щ':'O', 'З':'P',
				'Ф':'A', 'Ы':'S', 'В':'D', 'А':'F', 'П':'G', 'Р':'H', 'О':'J', 'Л':'K',
				'Д':'L', 'Я':'Z', 'Ч':'X', 'С':'C', 'М':'V', 'И':'B', 'Т':'N', 'Ь':'M',
				'х':'[', 'Х':'{', 'ъ':']', 'Ъ':'}', 'ж':';', 'Ж':':', 'э':'\'', 'Э':'"',
				'б':',', 'Б':'<', 'ю':'.', 'Ю':'>', ',':'?', '?':'&',
				'.':'/',
			}];

			static Invert(string, stickyMode = false) {
				let activeLayouts = this.Layouts;

				return `${string}`.split('').map(char => {
					activeLayouts.some(layout => {
						if(char in layout) {
							char = layout[char];

							if(stickyMode && activeLayouts.length > 1) {
								activeLayouts = [layout];
							}

							return true;
						}
					});

					return char;
				}).join('');
			}
		};
	}
}